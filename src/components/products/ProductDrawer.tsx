    ...
    <>
      ...
      <div className={cn('drawer', visible && 'open')}>
        ...
        <div className="px-5 py-5 flex flex-col gap-5">
          ...
          {/* Apartment number toggle — signs only */}
          {category === 'שלטי בית' && (
            <div className="flex items-center justify-between py-3 border-t border-cream-dark dark:border-navy-light">
              <div>
                <div className="text-sm font-medium">מספר דירה</div>
                <div className="text-xs text-muted mt-0.5">אפשרות להוסיף מספר דירה על השלט (עד 3 ספרות)</div>
              </div>
              <button
                onClick={() => setHasApartmentNumber(v => !v)}
                className={cn(
                  'w-11 h-6 rounded-full transition-colors relative shrink-0',
                  hasApartmentNumber ? 'bg-gold' : 'bg-cream-dark dark:bg-navy-light'
                )}
              >
                <div className={cn(
                  'w-5 h-5 rounded-full bg-white shadow absolute top-0.5 transition-all duration-200',
                  hasApartmentNumber ? 'right-0.5' : 'left-0.5'
                )} />
              </button>
            </div>
          )}
